...
export VERSION_ORIGINAL=$(apt-cache policy pgpool2 | grep Candidate | awk '{print $2}' | sed -E "s/(.*)[.].*$/\\1/")
cat ${VERSION_DIR}.bitnami_components.json | jq -M --arg VERSION_ORIGINAL "$VERSION_ORIGINAL" '.pgpool.version = $VERSION_ORIGINAL' > /tmp/components.json
