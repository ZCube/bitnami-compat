...
cat ../$(dirname $1)/prebuildfs/opt/bitnami/.bitnami_components.json | jq 'to_entries[] | "\(.key)-\(.value.version)"' | xargs -I {} -n 1 /bin/bash -c '[ -d "{}-${OS_NAME}-${OS_ARCH}-${OS_FLAVOUR}" ] || tar zxvf "{}-${OS_NAME}-${OS_ARCH}-${OS_FLAVOUR}.tar.gz"'
popd
